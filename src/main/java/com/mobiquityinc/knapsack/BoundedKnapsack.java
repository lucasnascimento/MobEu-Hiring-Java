...
/**
 * Solution by: http://rosettacode.org/wiki/Knapsack_problem/Bounded
 */
public class BoundedKnapsack extends ZeroOneKnapsack {
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (item.getBounding() > 1) {
                for (int j = 1; j < item.getBounding(); j++) {
                    add(item.getIndexAsString(), item.getWeightAsInteger(), item.getCostAsInteger());
                }
            }
        }
        ...
        while (itemList.size() > n) {
            ...
            if (lastItem.getInKnapsack() == 1) {
                for (int i = 0; i < n; i++) {
                    ...
                    if (lastItem.getIndexAsString().equals(iH.getIndexAsString())) {
    ...
    public void add(String name, int weight, int value, int bounding) throws APIException {
        if (name.equals(""))
