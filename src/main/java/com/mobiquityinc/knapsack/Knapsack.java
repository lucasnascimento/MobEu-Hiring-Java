    ...
    public Solution solve() {
        ...
        for (int i = 1; i <= NB_ITEMS; i++) {
            ...
            for (int j = 0; j <= capacity; j++) {
                if (items[i - 1].getWeightAsInteger() > j)
                    matrix[i][j] = matrix[i-1][j];
                else
                    // we maximize value at this rank in the matrix
                    matrix[i][j] = Math.max(matrix[i-1][j], matrix[i-1][j - items[i-1].getWeightAsInteger()]
                            + items[i-1].getCostAsInteger());
            }
        }
        ...
        for (int i = NB_ITEMS; i > 0  &&  res > 0; i--) {
            if (res != matrix[i-1][w]) {
                ...
                res -= items[i-1].getCostAsInteger();
                w -= items[i-1].getWeightAsInteger();
            }
        }
